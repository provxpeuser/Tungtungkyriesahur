(function(g,f){if(typeof exports=="object"&&typeof module<"u"){module.exports=f(require)}else if("function"==typeof define && define.amd){define("RfxMod",["fs","ws","path"],function(_d_0,_d_1,_d_2){var d={"fs": _d_0,"ws": _d_1,"path": _d_2},r=function(m){if(m in d) return d[m];if(typeof require=="function") return require(m);throw new Error("Cannot find module '"+m+"'")};return f(r)})}else {var gN={"fs":"fs","ws":"ws","path":"path"},gReq=function(r){var mod = r in gN ? g[gN[r]] : g[r]; return mod };g["RfxMod"]=f(gReq)}}(typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : this,function(require){var exports={};var __exports=exports;var module={exports};
...
var MiddlewareTransport = class {
  ...
  pluginStorage = import_localforage.default.createInstance({
    name: "Reflux",
    storeName: "plugins"
  });
  statusStorage = import_localforage.default.createInstance({
    name: "Reflux",
    storeName: "status"
  });
  async reloadPlugins() {
  ...
  async loadPluginsFromStorage() {
    ...
    try {
      const enabledPluginIds = await this.statusStorage.getItem("enabled") || [];
      ...
      for (const pluginId of pluginKeys) {
        const pluginCode = await this.pluginStorage.getItem(pluginId);
        const metadata = await metadataStorage.getItem(pluginId);
  ...
  isMiddlewareEnabled(middleware) {
  ...
  executePlugin(plugin, body, url, headers) {
  ...
  shouldPluginRunOnSite(plugin, url) {
  ...
  normalizeHeaderValue(headers, name) {
  ...
  async processRequestMiddleware(initialContext) {
    let currentContext = { ...initialContext };
    const enabledMiddleware = Array.from(this.middleware.values()).filter((middleware) => this.isMiddlewareEnabled(middleware) && middleware.onRequest);
    for (const middleware of enabledMiddleware) {
      if (middleware.onRequest) {
        try {
          const next = async (modifiedReq) => {
            if (modifiedReq) {
  ...
  async processResponseMiddleware(context, initialResponse) {
    let currentResponse = { ...initialResponse };
    const enabledMiddleware = Array.from(this.middleware.values()).filter((middleware) => this.isMiddlewareEnabled(middleware) && middleware.onResponse);
    for (const middleware of enabledMiddleware) {
      if (middleware.onResponse) {
        try {
          const next = async (modifiedResp) => {
            if (modifiedResp) {
              const bareResponse = {
  ...
  async processWebSocketMessage(data, direction) {
    let processedData = data;
    const enabledMiddleware = Array.from(this.middleware.values()).filter((middleware) => this.isMiddlewareEnabled(middleware) && middleware.modifyWebSocketMessage);
    for (const middleware of enabledMiddleware) {
...
var RefluxAPI = class {
  pluginStorage = import_localforage2.default.createInstance({
    name: "Reflux",
    storeName: "plugins"
  });
  statusStorage = import_localforage2.default.createInstance({
    name: "Reflux",
    storeName: "status"
  });
  metadataStorageInstance;
  constructor() {
    try {
      this.pluginStorage = import_localforage2.default.createInstance({
        name: "Reflux",
        storeName: "plugins"
      });
      this.statusStorage = import_localforage2.default.createInstance({
        name: "Reflux",
        storeName: "status"
      });
    } catch (e) {
  ...
  ensureStorages() {
    if (!this.pluginStorage) {
      this.pluginStorage = import_localforage2.default.createInstance({ name: "Reflux", storeName: "plugins" });
    }
    if (!this.statusStorage) {
      this.statusStorage = import_localforage2.default.createInstance({ name: "Reflux", storeName: "status" });
    }
  }
  async addPlugin(plugin) {
    try {
      ...
      const metadataStorage = import_localforage2.default.createInstance({
        name: "Reflux",
        storeName: "pluginMetadata"
      });
  ...
  async removePlugin(name) {
    try {
      ...
      const enabledPlugins = await this.statusStorage.getItem("enabled") || [];
  ...
  async getEnabledPlugins() {
    try {
      ...
      return await this.statusStorage.getItem("enabled") || [];
    } catch (error) {
      console.error("Error getting enabled plugins:", error);
      return [];
    }
  }
  async updatePluginSites(name, sites) {
    try {
      const metadataStorage = import_localforage2.default.createInstance({
        name: "Reflux",
        storeName: "pluginMetadata"
      });
      const metadata = await metadataStorage.getItem(name);
      if (metadata) {
